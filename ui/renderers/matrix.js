...
export function render(output) {
  ...
  const qHtml = (arr) => (Array.isArray(arr) ? arr.map(i => `<li>${formatItem(i)}</li>`).join('') : '<li>—</li>');
        ...
        <div class="quadrant-grid">
          ${['strengths','weaknesses','opportunities','threats'].map(key => `
            <div class="swot-box ${key}">
              ...
              <ul>${qHtml(a[key])}</ul>
              ${recs && recs[key] ? `<p class="recommendation">${escapeHtml(recs[key])}</p>` : ''}
            </div>
          `).join('')}
        </div>
        ${data.strategies ? `<div class="strategy-matrix"><h4>📈 Matrice Stratégique</h4><div class="strategies">${Object.entries(data.strategies).map(([k,v])=>`<div class="strategy"><strong>${escapeHtml(k)}:</strong> ${escapeHtml(v)}</div>`).join('')}</div></div>` : ''}
        ${data.recommendations && data.recommendations.next_steps ? `<div class="recommendations"><h4>Prochaines étapes</h4><ul>${data.recommendations.next_steps.map(s=>`<li>${escapeHtml(s)}</li>`).join('')}</ul></div>` : ''}
      </div>
    `;
  }
    ...
    <div class="result-container matrix-result">
      ...
      <div class="quadrant-grid">
        ${entries.map(([key, value]) => {
          const rawTitle = (data.quadrantLabels && data.quadrantLabels[key]) || key.replace(/[_-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
          const title = escapeHtml(rawTitle);
          const pct = dist[key] || dist[`${key}_pct`] || 0;
          return `
            <div class="quadrant ${escapeHtml(key)}">
              <h4>${title} (${pct}%)</h4>
              <ul>${qHtml(value)}</ul>
              ${recs && recs[key] ? `<p class="recommendation">${escapeHtml(recs[key])}</p>` : ''}
            </div>
