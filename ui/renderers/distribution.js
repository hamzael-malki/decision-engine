...
export function render(output) {
  ...
  const rows = entries.map(([k,v]) => `<div class="dist-row"><div class="label">${escapeHtml(k)}</div><div class="bar"><div class="bar-fill" style="width:${Math.min(100, Number(v) || 0)}%"></div></div><div class="val">${escapeHtml(v)}%</div></div>`).join('');
  return `
