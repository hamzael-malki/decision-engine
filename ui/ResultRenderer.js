...
export function renderResult(output) {
  try {
    const renderer = ResultRenderer[output.modelId] || ResultRenderer.fallback;
    return renderer(output);
  } catch (error) {
    console.error('Renderer error for model:', output.modelId, 'Output:', output, 'Error:', error);
    return `
      <div class="result-container fallback">
        <h3>❌ Erreur de Rendu</h3>
        <p><strong>Modèle:</strong> ${output.modelId}</p>
